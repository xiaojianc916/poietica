...
use crate::{HostError, Result, read_optional, write_atomic};
...
impl PluginLedger {
    ...
    pub fn set_enabled(&self, plugin_id: &str, enabled: bool) -> Result<()> {
        ...
        let at = index_of(rows, plugin_id)
            .ok_or_else(|| HostError::MissingPlugin(plugin_id.to_owned()))?;
...
mod tests {
    ...
    use super::{PluginInstall, PluginLedger};
    ...
    fn mutation_preserves_fields_owned_by_the_agent() {
        ...
        .expect("fixture");
        let ledger = PluginLedger::new(path.clone());
        ledger
    ...
    fn disabled_servers_are_read_from_the_official_shape() {
        ...
        .expect("fixture");
        let installed = PluginLedger::new(path)
            .installed()
