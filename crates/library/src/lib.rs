...
pub enum LibraryRequest {
    List { query: String },
    Read { path: String },
    Save { path: String, expected: String, content: String },
    Create { path: String, content: String },
    Folder { path: String },
    Trash { path: String, expected: String },
}
...
impl Vault {
    pub fn open(root: &Path) -> Result<Self> {
        ...
        if !root.is_dir() || root.to_str().is_none() {
            return Err(LibraryError::Invalid("请选择可读取的 UTF-8 路径文件夹。".into()));
        }
        Ok(Self { root })
    }
    ...
    fn path(&self, relative: &str, creating: bool) -> Result<PathBuf> {
        ...
        for (index, component) in components.iter().enumerate() {
            ...
            match fs::symlink_metadata(&result) {
                ...
                Ok(_) => {}
                Err(error) if creating && index + 1 == components.len()
                    && error.kind() == std::io::ErrorKind::NotFound => {}
                Err(error) => return Err(error.into()),
            }
        }
        let parent = result.parent().ok_or_else(|| LibraryError::Invalid("路径无父目录。".into()))?;
    ...
    fn note_path(&self, relative: &str, creating: bool) -> Result<PathBuf> {
        ...
        if !extension.is_some_and(|value| matches!(value.to_ascii_lowercase().as_str(), "md" | "markdown" | "txt")) {
            return Err(LibraryError::Invalid("此编辑器支持 .md、.markdown 与 .txt。".into()));
        }
        Ok(path)
    }
    ...
    fn read_at(path: &Path) -> Result<String> {
        if fs::metadata(path)?.len() > MAX_DOCUMENT_BYTES {
            return Err(LibraryError::Invalid("文件超过 16 MiB 编辑上限，未读取或修改。".into()));
        }
    ...
    fn document(&self, path: &str) -> Result<LibraryDocument> {
        ...
        Ok(LibraryDocument { path: path.to_owned(), content: Self::read_at(&absolute)? })
    }
    ...
    pub fn catalog(&self, query: &str) -> Result<LibraryCatalog> {
        ...
        for entry in walker {
            ...
            let path = relative.to_str().ok_or_else(|| LibraryError::Invalid("资料库含非 UTF-8 文件名。".into()))?.to_owned();
            let folder = entry.file_type().is_dir();
            if !folder {
                let supported = entry.path().extension().and_then(|value| value.to_str())
                    .is_some_and(|value| matches!(value.to_ascii_lowercase().as_str(), "md" | "markdown" | "txt"));
                if !supported { continue; }
                self.note_path(&path, false)?;
            }
            if !needle.is_empty() && !path.to_lowercase().contains(&needle) {
                if folder || !Self::read_at(entry.path())?.to_lowercase().contains(&needle) {
                    continue;
                }
            }
            let (modified, _, bytes) = file::read_file_metadata(entry.path()).map_err(LibraryError::Content)?;
            let name = entry.file_name().to_str().ok_or_else(|| LibraryError::Invalid("文件名不是 UTF-8。".into()))?.to_owned();
            let parent = relative.parent().and_then(|value| value.to_str()).unwrap_or("").to_owned();
            entries.push(LibraryEntry { path, folder, name, parent, modified: modified.map(|value| value.to_string()), bytes: bytes.to_string() });
            if entries.len() > MAX_ENTRIES {
                return Err(LibraryError::Invalid("结果超过 20,000 项，请选择更具体的文件夹；未返回不完整目录。".into()));
            }
        }
        entries.sort_by(|a, b| b.folder.cmp(&a.folder).then(a.path.cmp(&b.path)));
        Ok(LibraryCatalog { root: self.identity(), entries })
    }
    ...
    fn lock(&self) -> Result<File> {
        ...
        let lock = OpenOptions::new().read(true).write(true).create(true).truncate(false).open(path)?;
    ...
    fn write(&self, path: &str, content: &str, expected: Option<&str>) -> Result<LibraryDocument> {
        ...
        let parent = target.parent().ok_or_else(|| LibraryError::Invalid("路径无父目录。".into()))?;
        ...
        if let Some(expected) = expected {
            ...
            prepared.persist(&target).map_err(|error| LibraryError::Io(error.error))?;
        } else {
            prepared.persist_noclobber(&target).map_err(|error| LibraryError::Io(error.error))?;
        }
        Ok(LibraryDocument { path: path.to_owned(), content: content.to_owned() })
    }
    ...
    pub fn execute(&self, request: LibraryRequest) -> Result<LibraryReply> {
        match request {
            ...
            LibraryRequest::Save { path, expected, content } => self.write(&path, &content, Some(&expected)).map(LibraryReply::Document),
            LibraryRequest::Create { path, content } => self.write(&path, &content, None).map(LibraryReply::Document),
            ...
            LibraryRequest::Trash { path, expected } => {
                let _lock = self.lock()?;
                let absolute = self.note_path(&path, false)?;
                if !file::note_content_matches(&absolute, &expected).map_err(LibraryError::Content)? {
                    return Err(LibraryError::Conflict);
                }
                trash::delete(absolute).map_err(|error| LibraryError::Content(error.to_string()))?;
                Ok(LibraryReply::Done)
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn creates_reads_and_rejects_overwriting_existing_files() -> Result<(), Box<dyn std::error::Error>> {
        let root = tempfile::tempdir()?;
        let vault = Vault::open(root.path())?;
        vault.execute(LibraryRequest::Create { path: "笔记.md".into(), content: "正文".into() })?;
        assert!(vault.execute(LibraryRequest::Create { path: "笔记.md".into(), content: "覆盖".into() }).is_err());
        assert_eq!(fs::read_to_string(root.path().join("笔记.md"))?, "正文");
        assert!(matches!(vault.execute(LibraryRequest::Read { path: "笔记.md".into() })?, LibraryReply::Document(_)));
        Ok(())
    }

    #[test]
    fn external_changes_reject_stale_save_without_losing_either_version() -> Result<(), Box<dyn std::error::Error>> {
        let root = tempfile::tempdir()?;
        let vault = Vault::open(root.path())?;
        fs::write(root.path().join("note.md"), "outside")?;
        assert!(vault.execute(LibraryRequest::Save { path: "note.md".into(), expected: "before".into(), content: "draft".into() }).is_err());
    ...
    fn rejects_traversal_and_searches_multibyte_content() -> Result<(), Box<dyn std::error::Error>> {
        let root = tempfile::tempdir()?;
        let vault = Vault::open(root.path())?;
        assert!(vault.execute(LibraryRequest::Read { path: "../outside.md".into() }).is_err());
    ...
    fn does_not_follow_symbolic_links() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let vault = Vault::open(root.path())?;
        assert!(vault.execute(LibraryRequest::Read { path: "escape/secret.md".into() }).is_err());
