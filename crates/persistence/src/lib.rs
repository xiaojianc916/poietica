...
//!
//! 这个 crate 只回答五个问题：这台机器上有哪些对话、它们叫什么、各自握着
//! 谁的哪个会话、各自挂着哪些附件，以及每一轮从什么时候到什么时候。对话说过
//! 什么不在这里 —— 那份记录属于 agent，由 session/load 交还，那是唯一一份不会
//! 和别人漂移的历史。轮次的两端与附件同一类：不是内容，是这台机器上的事实。
//!
