...
impl<E: RuntimeFailure> Runtime<E> {
    ...
    where
        ...
        V: FnOnce(&poietica_ledger::index::AgentStore) -> Result<(), LedgerError>
            + Send + 'static,
        C: FnOnce() -> i64 + Send,
    {
        ...
        let live = self.ensure(request.agent_id, request.cwd, request.takeover)
            .await.map_err(CommandError::Runtime)?;
        let named = request.thread_id.to_string();
        let held = self.inner.sessions.resolve(
            &self.inner.index, &live.client, &live.book,
            &live.agent_id, &self.inner.root, &named,
        ).await.map_err(CommandError::Session)?;
        let expected_attachments = request.assets.len();
        let attachments = prepare_assets(held.thread_id, request.assets)
            .await.map_err(CommandError::Attachments)?;
        if attachments.len() != expected_attachments {
            return Err(CommandError::AttachmentSetChanged);
        }
        let session_id = held.session_id.clone();
        let model = request.configuration.iter()
            .find(|selection| selection.id == "model")
            .map(|selection| selection.value.clone()).unwrap_or_default();
        if !request.configuration.is_empty() {
            let controls = apply_configurations(
                &live.client, session_id.clone(), request.configuration,
                Some(request.text.clone()),
            ).await.map_err(CommandError::Agent)?;
        ...
        let prompt_id = submit(
            ...
            validate,
        ).await.map_err(CommandError::Delivery)?
            .ok_or(CommandError::MissingReceipt)?;
        drop(held);
        Ok(PromptReceipt { session_id, prompt_id })
    }
    ...
    pub async fn select_configuration(
        ...
        let held = match thread_id.as_deref() {
            Some(named) => Some(self.inner.sessions.resolve(
                &self.inner.index, &live.client, &live.book,
                &live.agent_id, &self.inner.root, named,
            ).await.map_err(CommandError::Session)?),
            None => None,
        };
        let addressed = held.as_ref()
    ...
    async fn announce(&self, live: &Handle, session_id: String, controls: Vec<ConfigControl>) {
        match live.client.goal(session_id.clone()).await {
            Ok(goal) => (self.inner.publish)(SessionEvent::Selectors { session_id, controls, goal }),
            Err(error) => {
                // Reporting failure does not undo an already accepted configuration.
                log::warn!("could not report the session goal after a configuration change: {error}");
            }
        }
    }
}
