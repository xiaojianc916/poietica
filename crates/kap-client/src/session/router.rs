//! 事件路由：ack 的订阅屏障与 resync 的快照恢复；审批/提问的 REST 对账在 reconcile.rs。
...
impl EventRouter {
    ...
    pub(crate) fn handle(&mut self, envelope: &Value) {
        // 事件信封的 type 就是事件自己的 type；载荷无同值 type 的控制帧与系统帧收不进来（wsEventEnvelopeSchema）。
        ...
        match event_type {
            ...
            "event.session.work_changed" => {
                /* 审批没有事件之外的可靠清单通知：聚合报 pending_interaction=approval 时由对账任务拉 REST 清单。 */
                ...
                /* work_changed 是活动投影，不是轮终错误通道：仅在聚合落定后推进 durable cursor，同轮稍后的 error 事件也在续订水位内。 */
            ...
            "agent.status.updated" => {
                // 仪表值 volatile（不进帧日志）、到达即替换；累计输入构成在 usage.total（kap events-zod.ts），这条协议知识全程只有这一处。
