//! 把裸程序名解析成本机真的能启动的一条启动式。
//!
//! mcp.json 里 stdio 条目的 command 是 kap 那一侧要去 spawn 的程序，裸名字在那里不是
//! 可启动的对象（平台事实与唯一解析处见 crates/kap-client/src/process/program.rs），
//! 所以在写盘那一刻就把解析结果固化进条目：绝对路径；Windows 的 .cmd/.bat 垫片由
//! cmd.exe /c 代起——VS Code 与 Claude Desktop 的官方 Windows 文档给 stdio MCP 服务器
//! 写的也是 cmd /c。解不出来的条目就不该被写进去。
...
/// 解析一台 stdio MCP 服务器的启动器。本机没有该程序时返回 `None` 而非错误：缺程序
/// 是那台机器的现状，界面要把这句话说出来，而不是弹一次错误。
#[command]
#[specta::specta]
