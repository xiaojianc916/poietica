...
//!
//! 代价照实说：名单跟着内置表走，但不抄表 —— 只有 /models 真校验密钥的主机才进来
//! （tokenrouter 于 2026-08-31、b.ai 于 2026-09-01 进内置表时都按决定不进名单，
//! 谁也没有验证过它们 /models 的行为；放一个「任何钥匙都回 200」的主机进来，就是
//! 把「未验证」说成「已验证」）。失败是关闭的 —— 名单里没有就回 Unsupported
//! （「没能验证」），不会变成一次错误的「密钥不对」，更不会把密钥发到名单外的地方。
//!
