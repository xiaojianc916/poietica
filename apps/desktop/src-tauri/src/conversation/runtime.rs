...
impl AgentRuntime {
    ...
    /// The host constructs paths and persistence before transferring ownership here.
    #[must_use]
    pub fn new(root: PathBuf, attachments: PathBuf, journal: FrameJournal) -> Self {
        Self {
            attachments,
            root,
            journal,
...
pub(super) async fn ensure_session(
    ...
    std::thread::spawn(move || {
        ...
        local.block_on(&rt, async move {
            let (driver, events) = tokio::join!(driver_task, events_task);
            for (task, outcome) in [("driver", driver), ("events", events)] {
                if let Err(error) = outcome {
                    log::error!("agent {task} task failed: {error}");
                }
            }
        });
    });
    ...
    async_runtime::spawn(async move {
        let index = ledger.state::<crate::ledger::LocalIndex>();
    ...
    async_runtime::spawn(async move {
        let index = herald.state::<crate::ledger::LocalIndex>();
        ...
        match poietica_conversation_runtime::recover(
            index.inner(),
            gateway,
            &drain_owner,
            &runtime.sessions,
