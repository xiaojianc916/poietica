...
describe('computerUseFailureDescription', () => {
  it('preserves upstream PowerShell diagnostics instead of inventing recovery', () => {
    const reason =
      'Oh My Pi Computer Use requires Windows PowerShell 5.1 or PowerShell 7 with the commands required by its official installer. Windows PowerShell: missing commands: Get-FileHash; PowerShell 7: spawn ENOENT'
  ...
  it('does not render an empty failure', () => {
    expect(computerUseFailureDescription('   ')).toBe('安装失败：Oh My Pi 未提供失败原因。')
  })
})
