...
describe('resolveMcpServers', () => {
  it('mcp.json 里那些排在前，插件带来的在后，都由命令行起', () => {
    const resolved = resolveMcpServers({
      environment: [declared('from-config', true)],
      plugins: [plugin('demo', { mcpServerNames: ['from-plugin'] })],
    })

    expect(resolved.map((server) => server.name)).toEqual(['from-config', 'from-plugin'])
    expect(resolved.map((server) => server.launchedBy)).toEqual(['agent', 'agent'])
  })
